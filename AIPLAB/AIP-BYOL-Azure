...
#-DefaultLocationName $azureDefaultLocation
...
Add-LabMachineDefinition -Name VictimPC -OperatingSystem $ClientOS -IpAddress 192.168.41.52 -Network $labName -PostInstallationActivity $postInstallActivity -Domain contoso.azure
...
Install-LabSoftwarePackage -Path $labSources\SoftwarePackages\AzInfoProtection_UL.exe -CommandLine /S -ComputerName VictimPC
...
Invoke-LabCommand -ScriptBlock { Set-Location "C:\Program Files\Common Files\microsoft shared\ClickToRun\"; .\OfficeC2RClient.exe /update user } -ComputerName AdminPC,ClientPC,VictimPC
