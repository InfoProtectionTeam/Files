...
'$DelegatedUser = Get-Credential -Message "Enter the cloud AIP scanner service account credentials"' | Out-File ~\Desktop\Set-AIPAuthenticationUL.ps1 -append
"Set-AIPAuthentication -AppID " + $WebApp.AppId + " -AppSecret " + $WebAppKey.Guid + " -TenantID " + $TenantID.Guid + ' -DelegatedUser $DelegatedUser -OnBehalfOf $ServiceAccount' | Out-File ~\Desktop\Set-AIPAuthenticationUL.ps1 -append
